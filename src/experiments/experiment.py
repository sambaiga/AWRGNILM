...
from data_generator import get_loaders
...
def experiments_two(dataset="lilac",
                            image_type="adaptive",
                            width=50,
                            run_id=1,
                            epochs=100,
    ...
    current, voltage, labels = get_data(submetered=submetered, data_type=dataset, isc=isc)
    ...
    train_model_stratified(input_feature, y, model_name, epochs=epochs, file_name=file_name, image_type=image_type, in_size=in_size)
...
if __name__ == "__main__":
    ...
    for dataset in  ["plaid"]:
        for image_type in ["adaptive", "vi"]:
            experiments_two(dataset=dataset,
